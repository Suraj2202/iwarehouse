...
router.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        if(buycount){newProduct.buycount = buycount};
        if(offercount){newProduct.offercount = offercount};
        ...
        if(product.user.toString() !== req.user.id){return res.status(401).send("Unauthorized access, not allowed.")}
